...
impl FlatBufferBuilder {
    ...
    // 要生成 “name” 对应的结构体的 flatbuffer结构
    // pub fn build<T>(&mut self, messages: &mut FlatBufferMessages<T>, name: &str)
    //     -> Result<FlatBuffer, &str>
    //     where T: Clone
    // {
    //     let message = match messages.find(name) {
    //         Ok(m) => m,
    //         Err(e) => return Err(e),
    //     };// 得到一个 flatbuffer message
    //     let mut flatbuffer = FlatBuffer::with_pivot(message.list.len() as u8);
    //     // 准备填充
    //     let mut index = 0;
    //     for flatbuffer_type in &mut message.list {
    //         // 判断是否有值，没有值直接进行下一项
    //         let value = match flatbuffer_type.get_v() {
    //                 Some(v) => v,
    //             None => { index += 1; continue;},
    //         };
    //         // 获取到每一个 FlatBufferType
    //         match flatbuffer_type.get_t() {
    //             FBType::FBBool => {
    //                 let value: bool = value;
    //                 self.add_bool(&mut flatbuffer, index, value);
    //             },
    //             // self.add_bool(&mut flatbuffer, index, value),
    //             FBType::FBString => 
    //             self.add_string(&mut flatbuffer, index, value),
    //             FBType::FBList =>
    //             self.add_list(&mut flatbuffer, index, value),
    //             FBType::FBMessage => {
    //                 let (bytes, pivot) = match self.build(messages, value) {
    //                     Ok(flatbuffer) => flatbuffer.bytes(), // 整合到 flatbuffer 中
    //                     Err(e) => (None, 0),
    //                 };
    //                 let value = bytes.unwrap();
    //                 self.add_flatbuffer_bytes(&mut flatbuffer, index, value, pivot as u32);
    //             },
    //         };
    //         index += 1;
    //     }
